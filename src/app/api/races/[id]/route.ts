...
export async function GET(
  ...
  try {
    ...
    const buildIds = raceCars?.map((rc) => rc.buildId as string).filter(Boolean) || []
    ...
    for (const lapTime of lapTimes || []) {
      const user = (lapTime as { user: { id: string; name: string; email: string }[] })?.user?.[0]
      const car = (lapTime as { car: { id: string; name: string; slug: string; manufacturer: string; year: number }[] })?.car?.[0]
      const userId = user?.id
      const carId = car?.id
      const buildId = (lapTime as { buildId: string }).buildId || null
      const buildName = (lapTime as { buildName: string }).buildName || null
    ...
    if (session?.user?.email) {
      ...
      if (userData) {
        const userLapTimes = (lapTimes || []).filter((lt) => (lt as { user: { id: string }[] })?.user?.[0]?.id === userData.id)
